...
$(document).ready(function(){
   jQuery.fn.makeItBounce = function(){
     ...
     //Let's do some style
     this.width(1400).height(600).css('background-color', '#000');

     for(var i=0; i<140;i++){
       //Let's add something
       this.append('<ball></ball>');
       var ball = this.find('ball').eq(0);
       ball.width(100).height(100).css('background-color', '#fff').css('display','inline-block').css('border-radius','50px').css('position','absolute').css('top',285);

       var x,y,affX,affY;
       var initX = parseInt(ball.css('left'));
       var initY = parseInt(ball.css('top'));

       var rdX = Math.floor(Math.random() * 50) + 15;
